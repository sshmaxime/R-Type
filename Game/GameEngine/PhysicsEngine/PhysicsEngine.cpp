...
namespace TacosEngine
{
    ...
    void PhysicsEngine::update(std::list<std::shared_ptr<GameObject>> &objects) {
        ...
        for (auto &it : objects) {
            ...
            if (body && body->isActive()) {
                ...
                float x = it->getTransform().getPosition().get_x();
                float y = it->getTransform().getPosition().get_y();
                float xNew = newPos.get_x();
                float yNew = newPos.get_y();
    ...
    bool PhysicsEngine::checkCollision(std::shared_ptr<GameObject> s1, const Vector2 &force,
        ...
        if (posS1.get_x() < posS2.get_x() + sizeS2.get_x() &&
            ...
            callOnCollide(objects, std::move(s1), std::move(s2));
            return true;
        }
        return false;
    }
